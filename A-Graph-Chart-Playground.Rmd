...
number_of_source_records <- nrow(df)
...
colnames(only_efficient_stocks_MM) <- c("DMU","Score", "X1", "X2", "X3", "X4",
                                        "Y1", "Y2", "Y3", "Y4", "Y5")
...
number_of_efficient_stocks <- nrow(only_efficient_stocks_MM)
...
df_MM_io_data <- df[df$DMU %in% only_efficient_stocks_MM,]
...
`r SECTOR` Sector Data Size: `r number_of_source_records`

100% Efficient Stocks (DEA): `r number_of_efficient_stocks`

Stocks Selected Percentage: `r (number_of_efficient_stocks/number_of_source_records)*100`%


### Efficient Stocks for Portfolio

**Stock portfolio generated from DEA analysis:**

```{r ShowEfficientStocks, echo=FALSE, message=FALSE}

# SHOW EFFICIENT STOCKS RETURNED FROM THE DEA MODEL
kbl (only_efficient_stocks_MM[,1:2], booktabs=T, escape=F, digits=2, longtable=T,
     col.names=c("DMU", "Efficiency Score"),
     caption =c("Multiplier Model Stock Portfolio Results")) |>
...
for the review of source data.*

```{r MultiplierModelData, echo=FALSE, message=FALSE}

kbl (df_MM_io_data, booktabs=T, escape=F, digits=2, longtable=F,
     caption =c("MM Input and Output Data Used for Analysis"))  |>
  add_header_above(c(" " = 2,  "Inputs (x)" = 4, "Outputs (y)" = 5, "Prices" = 4, "Sector" = 1))  |>
  kable_styling (latex_options = c("HOLD_position", "scale_down"))
...
temp_MM <- only_efficient_stocks_MM[,1:2]

# NAME THE TEMP COLUMNS
colnames(temp_MM) <- c("DMU","Score")

# SLICE OFF ONLY THE STOCKS THAT HAVE AN EFFICIENCY OF ONE (1)
only_efficient_stocks_RETURNS <- temp_MM[temp_MM[,2] >= 1.0,]

# MERGE: INNER JOIN THE DATAFRAMES FOR BACKTESTING VALUES AND EFFICIENT DMUs
df_all = merge(x = only_efficient_stocks_RETURNS, y = returnResults, by = "DMU")
...
## Source Input Data (X Dataset)

```{r ShowOutputData, echo=FALSE, message=FALSE}

x_out <- cbind(df['DMU'], x)
...
```{r ShowInputData, echo=FALSE, message=FALSE}

y_out <- cbind(df['DMU'], y)
...
```{r ShowAllData, echo=FALSE, message=FALSE, eval=FALSE}
...
# SHOW ALL OF THE SECTOR DATA USED FOR ANALYSIS
# THIS OUTPUT TRUNCATES AT THE END OF THE PAGE, LONGTABLE CAUSES ERROR W/SCALE_DOWN
#kbl(df, booktabs=T, digits=3, escape=T, row.names=F, longtable=F,
#    col.names=c("DMU","Beta", "Sigma", "Debt To Equity","Price to Book", "Net Margin", "EPS", "ROA", "ROE", "TMret", #"Entry", "Period 1", "Period 2", "Period 3", "Sector"),
#    caption= 'All Source Data Used for DEA Analysis')  |>
#  kable_styling(latex_options = c("HOLD_position", "scale_down"))

# SHOW THE HEAD/TAIL INPUT VARIABLE EXAMPLES
#kbl(xxd, booktabs=T, digits=3, escape=T, row.names=F, longtable=F,
#    col.names=c("DMU","Beta", "Sigma", "Debt To Equity","Price to Book"),
#    caption= 'Data Source Extract Example - INPUTS ($x$)')  |>
#  kable_styling(latex_options = c("HOLD_position", "scale_down"))

# SHOW THE HEAD/TAIL OUTPUT VARIABLE EXAMPLES
