...
# limit=number
# 
# https://financialmodelingprep.com/api/v3/key-metrics/AAPL?period=annual | quarter
...
# FMP REST API URLs
base_url = 'https://financialmodelingprep.com/api/v3/key-metrics/'  # ADD SYMBOL ON END OF URL
...
limit = '32'                   # MAX NUMBER OF PERIODS TO RETURN: MAX = 140
##########
...
stock_list = pd.read_sql('SELECT DISTINCT symbol FROM exchange_symbols WHERE Symbol NOT LIKE "%-%" AND Symbol NOT LIKE "%.%" AND Symbol NOT LIKE "%^%" ORDER BY symbol', engine1)
##########
...
for x in stock_list['symbol']:
  ...
  df.to_sql(name='company_eps_pe', con=engine2, chunksize=5000, if_exists='append') # ADD DATA TO EXISTING TABLE
